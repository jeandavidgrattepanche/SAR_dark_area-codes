...
#					print("no family", order, family)
	...
	for ord1, fam in taxo4dict.items():
		tree = ""
		number=0;ordernumber=0;numberofseq=0;ordseq=0;extinct=0
		for fa in fam:
			if fa not in falist:
				falist.append(fa)
				number=(int(str(taxo5dict[fa]).count(','))+1)-int(str(taxo5dict[fa]).count('-extinct'))
				extinct = int(str(taxo5dict[fa]).count('-extinct'))
				numberofseq=int(str(taxo5dict[fa]).count('-GBank'))
				if tree == "":
					try: 
						tree = "("+str(taxo5dict[fa]).split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+":0)"+'{'+str(int(numberofseq)/int(number))+'}'
					except:
						tree =fa.split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+':0'
				else:
					try: 
						tree = tree+",("+str(taxo5dict[fa]).split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+":0)"+'{'+str(int(numberofseq)/int(number))+'}'
					except:
						tree = tree+',' + fa.split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+':0'
				outtable = open('number.txt','a')
				outtable.write(str(fa)+'\t'+str(number) + '\t'+ str(extinct) + '\t' + str(numberofseq) + '\n')
				outtable.close()
				ordernumber=ordernumber + number
				ordseq=ordseq+numberofseq
		tree1 = '('+ tree + ')'+'{!'+str(ordernumber)+'/'+str(ordseq)+'!}' #+ ord1
#		print(tree1)
		ordtree[ord1] = tree1
		if tree1 == "":
			print(ord1)
	...
	for Cl1, ord in taxo3dict.items():
		tree1 = "empty"
		number=0;clnumber=0;numberofseq=0;clseq=0
		for orde in ord:
...
#				print(Cl1, orde)
				if orde.split('_')[0] == "or":
					...
					numberofseq=int(ordtree[orde].split('/')[-1].split('!}')[0])
#					print(ordtree[orde], number)
					if tree1 == "empty":
						try: 
							tree1 = str(ordtree[orde].split('{!')[0])+'{'+str(int(numberofseq)/int(number))+'}'
							print(ordtree[orde], ordtree[orde].split('{!')[0])
						except:
							print('error order1')
#							tree = '(' + fa + ')' 
					else:
						try: 
							tree1 = tree1+","+str(ordtree[orde].split('{!')[0])+'{'+str(int(numberofseq)/int(number))+'}'
#							print(tree1)
						except:
							print('error order2')
					...
					if orde not in falist:
						falist.append(orde)
						number=(int(str(taxo5dict[orde]).count(','))+1)-int(str(taxo5dict[orde]).count('-extinct-'))
						numberofseq=int(str(taxo5dict[orde]).count('-GBank'))
						if tree1 == "empty":
							try: 
								tree1 = "("+str(taxo5dict[orde]).split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+":0)"+'{'+str(int(numberofseq)/int(number))+'}'
							except:
#								tree1 = '(' + orde + ')' 
								print('error fa1')
						else:
							try: 
								tree1 = tree1+",("+str(taxo5dict[orde]).split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+":0)"+'{'+str(int(numberofseq)/int(number))+'}'
#								print(tree1)
							except:
#								tree1 = tree1+',(' + orde + ')' 
								print('error fa2')
				...
				clseq=clseq+numberofseq
#				print("clnumber:",clnumber)
		tree2= '(' + tree1 + '){!'+str(clnumber)+'/'+str(clseq)+'!}' #+ Cl1
		if tree1=="empty":
			print(tree2)
		cltree[Cl1]=tree2
		print(tree2)
		
	phtree= {};cllist=[];perc=0
	for ph1, cl in taxo2dict.items():
		tree2 = ""
		tree1 = ""
		number=0;phnumber=0;numberofseq=0;phseq=0
		for cle in cl:
...
#				print(ph1, cle)
				if cle.split('_')[0] == "cl":
					...
					numberofseq=int(cltree[cle].split('/')[-1].split('!}')[0])
					if tree1 == "":
						try: 
							tree1 = str(cltree[cle].split('{!')[0])+'{'+str(int(numberofseq)/int(number))+'}'
#							print(cle, "=>",tree1)
						except:
							print('error cl1')
#							tree = '(' + fa + ')' 
						...
						try: 
							tree1 = tree1+","+str(cltree[cle].split('{!')[0])+'{'+str(int(numberofseq)/int(number))+'}'
						except:
							print('error cl2')
				elif cle.split('_')[0] == "or":
					number=int(ordtree[cle].split('{!')[-1].split('/')[0])
					numberofseq=int(ordtree[cle].split('/')[-1].split('!}')[0])
					if cle not in ordlist:
						ordlist.append(cle)
						if tree1 == "":
							try: 
								tree1 = "("+str(ordtree[cle].split('{!')[0])+'{'+str(int(numberofseq)/int(number))+'}'+")"#+cle
#								print(cle, '=>', tree1)
							except:
#								tree1 = '(' + orde + ')' 
								print('error clor1')
							...
							try: 
								tree1 = tree1+",("+str(ordtree[cle].split('{!')[0])+'{'+str(int(numberofseq)/int(number))+'}'+")"#+cle
							except:
#								tree1 = tree1+',(' + orde + ')' 
								print('error clor2')
					...
					if cle not in falist:
						falist.append(cle)
						number=(int(str(taxo5dict[cle]).count(','))+1)-int(str(taxo5dict[cle]).count('-extinct-'))
						numberofseq=int(str(taxo5dict[cle]).count('-GBank'))
						if tree1 == "":
							try: 
								tree1 = "("+str(taxo5dict[cle]).split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+":0)"+'{'+str(int(numberofseq)/int(number))+'}'
#								print(cle, '=>', tree1)
							except:
#								tree1 = '(' + orde + ')' 
								print('error clfa1')
							...
							try: 
								tree1 = tree1+",("+str(taxo5dict[cle]).split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+":0)"+'{'+str(int(numberofseq)/int(number))+'}'
							except:
#								tree1 = tree1+',(' + orde + ')' 
								print('error clfa2')
				else:
				...
				phseq=phseq+numberofseq
		perc=int(phseq)/int(phnumber)		
#		tree2= '(' + tree1 + '){'+str(perc)+'}' #+ ik1
		tree2= '(' + tree1 + '){!'+str(phnumber)+'/'+str(phseq)+'!}'# + ph1
#		print(tree2)
		phtree[ph1]=tree2
...
#		out1.close()
	...
	for ik1, ph in taxo1dict.items():
		tree2 = ""
		number=0;iknumber=0;numberofseq=0;ikseq=0
		for phe in ph:
...
#				print(ik1, phe)
				if phe.split('_')[0] == "ph":
					...
					numberofseq=int(phtree[phe].split('/')[-1].split('!}')[0])
					if tree1 == "":
						print(ik1)#, phtree[phe].split('{')[0])
						try: 
							tree1 = str(phtree[phe].split('{!')[0])+'{'+str(int(numberofseq)/int(number))+'}'
						except:
							print('error ph1')
#							tree = '(' + fa + ')' 
					else:
						try: 
							tree1 = tree1+","+str(phtree[phe].split('{!')[0])+'{'+str(int(numberofseq)/int(number))+'}'
						except:
							print('error ph2')
				elif phe.split('_')[0] == "cl":
					number=int(cltree[phe].split('{!')[-1].split('/')[0])
					numberofseq=int(cltree[phe].split('/')[-1].split('!}')[0])
					if tree1 == "":
						print(ik1)
						try: 
							tree1 = str(cltree[phe].split('{!')[0])+'{'+str(int(numberofseq)/int(number))+'}'
						except:
							print('error phcl1')
#							tree = '(' + fa + ')' 
					else:
						try: 
							tree1 = tree1+","+str(cltree[phe].split('{!')[0])+'{'+str(int(numberofseq)/int(number))+'}'
						except:
							print('error phcl2')
				elif phe.split('_')[0] == "or":
					number=int(ordtree[phe].split('{!')[-1].split('/')[0])
					numberofseq=int(ordtree[phe].split('/')[-1].split('!}')[0])
					if tree1 == "":
						print(ik1)
						try: 
							tree1 = str(ordtree[phe].split('{!')[0])+'{'+str(int(numberofseq)/int(number))+'}'
						except:
#							tree1 = '(' + orde + ')' 
							print('error phor1')
					else:
						try: 
							tree1 = tree1+","+str(ordtree[phe].split('{!')[0])+'{'+str(int(numberofseq)/int(number))+'}'
						except:
#							tree1 = tree1+',(' + orde + ')' 
							print('error phor2')
				elif phe.split('_')[0] == "fa":
					number=(int(str(taxo5dict[phe]).count(','))+1)-int(str(taxo5dict[phe]).count('-extinct-'))
					numberofseq=int(str(taxo5dict[phe]).count('-GBank'))
					if tree1 == "":
						print(ik1)
						try: 
							tree1 = "("+str(taxo5dict[phe]).split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+":0)"+'{'+str(int(numberofseq)/int(number))+'}'
						except:
#							tree1 = '(' + orde + ')' 
							print('error phfa1')
					else:
						try: 
							tree1 = tree1+",("+str(taxo5dict[phe]).split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+":0)"+'{'++'/'+str(int(numberofseq)/int(number))+'}'
						except:
#							tree1 = tree1+',(' + orde + ')' 
							print('error phfa2')
				else:
				...
				ikseq=ikseq+numberofseq
		perc=int(ikseq)/int(iknumber)		
		tree2= '(' + tree1 + '){'+str(perc)+'}' #+ ik1
		tree1=""
#		print(tree2)
