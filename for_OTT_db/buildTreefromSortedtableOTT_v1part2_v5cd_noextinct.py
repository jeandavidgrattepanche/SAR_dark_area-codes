...
#		species = "sp_"+line.split('\t')[7].split('\n')[0].replace(':','-').replace('(','').replace(')','').replace('/','')
		for speciese in line.split('\t')[7].split('\n')[0].replace(':','-').replace('(','').replace(')','').replace('/','').split(','):
			if not '-extinct' in speciese:
				if species == "":
					species='sp_'+speciese
				else:
					species=species+','+speciese		
#		print(species)		
	...
	for ord1, fam in taxo4dict.items():
		...
		for fa in fam:
			if fa not in falist:
				...
				if tree == "":
					try: 
						tree = "("+str(taxo5dict[fa]).split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+":0)"+'{'+str((int(str(taxo5dict[fa]).count(','))+1)-(int(str(taxo5dict[fa]).count('-extinct-'))))+'}'
					except:
						tree =fa.split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+':0'
				else:
					try: 
						tree = tree+",("+str(taxo5dict[fa]).split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+":0)"+'{'+str((int(str(taxo5dict[fa]).count(','))+1)-(int(str(taxo5dict[fa]).count('-extinct-'))))+'}'
					except:
...
#							tree = '(' + fa + ')' 
					...
					if orde not in falist:
						...
						if tree1 == "empty":
							try: 
								tree1 = "("+str(taxo5dict[orde]).split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+":0)"+'{'+str((int(str(taxo5dict[orde]).count(','))+1)-int(str(taxo5dict[orde]).count('-extinct-')))+'}'
							except:
#								tree1 = '(' + orde + ')' 
								print('error fa1')
						else:
							try: 
								tree1 = tree1+",("+str(taxo5dict[orde]).split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+":0)"+'{'+str((int(str(taxo5dict[orde]).count(','))+1)-int(str(taxo5dict[orde]).count('-extinct-')))+'}'
#								print(tree1)
							except:
#								tree1 = tree1+',(' + orde + ')' 
								print('error fa2')
...
#								tree1 = tree1+',(' + orde + ')' 
					...
					if cle not in falist:
						...
						if tree1 == "":
							try: 
								tree1 = "("+str(taxo5dict[cle]).split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+":0)"+'{'+str((int(str(taxo5dict[cle]).count(','))+1)-int(str(taxo5dict[cle]).count('-extinct-')))+'}'
#								print(cle, '=>', tree1)
							except:
#								tree1 = '(' + orde + ')' 
								print('error clfa1')
							...
							try: 
								tree1 = tree1+",("+str(taxo5dict[cle]).split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+":0)"+'{'+str((int(str(taxo5dict[cle]).count(','))+1)-int(str(taxo5dict[cle]).count('-extinct-')))+'}'
							except:
#								tree1 = tree1+',(' + orde + ')' 
								print('error clfa2')
				else:
...
#							tree1 = tree1+',(' + orde + ')' 
					...
					if tree1 == "":
						print(ik1)
						try: 
							tree1 = "("+str(taxo5dict[phe]).split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+":0)"+'{'+str((int(str(taxo5dict[phe]).count(','))+1)-int(str(taxo5dict[phe]).count('-extinct-')))+'}'
						except:
#							tree1 = '(' + orde + ')' 
							print('error phfa1')
					else:
						try: 
							tree1 = tree1+",("+str(taxo5dict[phe]).split(',')[0].replace("'","").replace('[','').replace(']','').replace('"',"").replace(',',':0,')+":0)"+'{'+str((int(str(taxo5dict[phe]).count(','))+1)-int(str(taxo5dict[phe]).count('-extinct-')))+'}'
						except:
#							tree1 = tree1+',(' + orde + ')' 
							print('error phfa2')
				else:
