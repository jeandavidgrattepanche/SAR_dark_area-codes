...
def main():
	...
	i=0;namelist = []; gbdict= {}; sourcedict = {}; GPSudict = {} ; titledict = {}; countrydict = {}; GPSdict = {}; valuelist = []; gbtokeep= []
	out = open(dbfile.split('.txt')[0]+ "_locationv2.txt",'w+'); out2 = open('issue_location.txt','w+')
		...
		if line[0] == '>':
			if line.split('\t')[1] in gbtokeep:
				...
				if name not in namelist:
					...
					try:
						countrydict[name].append(country.split(':')[0])
					except:
...
#								print(name)
	...
	for species in namelist:
		out.write(species + '\t' + str(len(gbdict[species]))+'\t'+ str(len(titledict[species]))+'\t'+ str(len(GPSdict[species]))+'\t'+ str(len(GPSudict[species]))+'\t' + str(gbdict[species]) +'\t' + str(GPSudict[species]) + '\t' + str(countrydict[species]) + '\t' + str(titledict[species]) + '\n')
	out.close()
